...
var NCWebSDK = (() => {
  ...
  var DEFAULT_CONFIG = {
    ...
    sessionValidation: (token) => token === "67421e9ef775687157024d3eb056193d:675fa9789d58913f86f4530b769b234a",
    // Default validation
    sessionCheckInterval: 5 * 60 * 1e3
    // 5 minutes
  };
  ...
  function initialize({ getSessionId, config }) {
    ...
    const createIframeWithSource = (url) => {
      return new Promise((resolve, reject) => {
        ...
        let iframe = document.getElementById(DEFAULT_CONFIG.iframeId);
        if (iframe) {
    ...
    const connect = async (componentName) => {
    ...
    const logout = async () => {
      try {
        const response = await axios_default.post(`${DEFAULT_CONFIG.baseUrls.backendServer}/auth/access-token/revoke`, {
  ...
  // src/cdn-index.ts
  globalThis.NCWebSDK = initialize;
})();
